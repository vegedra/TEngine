...
static bool getInput(void) {
	...
	/* a função fgets() lê uma linha de texto da entrada padrão (stdin) e armazena essa linha
	na variável input, cujo tamanho é determinado por 'sizeof input'. A entrada padrão stdin 
	representa o teclado. A expressão != NULL verifica se a função fgets() teve sucesso na leitura da linha. 
	Se sim, retorna um ponteiro para a string lida, caso contrário, retorna NULL. Ou seja, verifica se
	uma linha de texto foi lida com sucesso da entrada padrão e retorna 1 se sim, ou 0 se não.  */
	return fgets(input, sizeof input, stdin) != NULL;  // Função fgets (standard) pega input do teclado. 
}

// Função main() é o ponto inicial de um programa em C e que chama as outras funções. O void é pq n tem parametros
int main(void) {  // Função main() é o ponto inicial de um programa em C e que chama as outras funções.   
